...
% Jean-Yves Tinevez <tinevez@pasteur.fr> March 2012
    ...
    methods
        ...
        function flag = isleaf(obj, ID)
           ...
           if any(ID < 1) || any(ID > numel(obj.Parent)),
                error('MATLAB:tree:isleaf', ...
                    'No node with ID %d.', ID)
           end
           
	   flag = zeros(1,length(ID));
	   for i=1:length(ID),
		   %parent = obj.Parent;
		   flag(i) = ~any( obj.Parent == ID(i) );
	   end
        ...
        function IDs = getsiblings(obj, ID)
            ...
            % given node ID. (including ID itself)
	    ...
	    children = obj.getchildren( obj.Parent(ID) );
            IDs = children(children ~= ID);
        end
