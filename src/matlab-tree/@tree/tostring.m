...
% is true. It is false by defualt.
    ...
    if ~isstruct(obj.Node{1}),
	    strContentTree = obj.treefun(@contentToString);
    else %NPAV
	    strContentTree = tree(obj, 'clear'); 
	    strContentTree.Node = split(num2str([1:length(obj.Node)])); %cellfun(fun, obj.Node, 'UniformOutput', false);
    end
    ...
    for i = iterator
       ...
       nSiblings = numel(spaceTree.getsiblings(i))+1;
